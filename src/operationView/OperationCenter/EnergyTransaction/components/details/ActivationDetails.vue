...
const dialogStatusMap: Record<number, string> = { 1: '已完成', /* Other statuses from screenshot/data? */ };
const dialogStatusColorMap: Record<number, string> = { 1: 'success', /* Other statuses from screenshot/data? */ };
  ...
  try {
    ...
    if (response && response.code === '000000' && response.data) {
        ...
        if (!Array.isArray(list)) {
             console.warn('getBatchActiveListApi (for activation) response.data.list is not an array', response)
        }
    } else {
...
const activationTableColumns = ref<TableColumn[]>([
  ...
  },
  {
    ...
    prop: 'handle_status', field: 'handle_status', label: '状态', width: 120, align: 'center',
    slots: { default: ({ row }) => getStatusTag('handle_status', row.handle_status) }
  },
  {
    // Assuming "激活时间" corresponds to create_time
    prop: 'create_time', field: 'create_time', label: '激活时间', width: 160,
    formatter: (row) => formatDisplayDateTime(row.create_time)
  },
  {
    prop: 'finish_time', field: 'finish_time', label: '完成时间', width: 160,
    formatter: (row) => formatDisplayDateTime(row.finish_time)
  },
  {
    prop: 'action', field: 'action', label: '操作', width: 120, align: 'center', fixed: 'right',
    slots: {
      default: ({ row }) => {
        return h(
          ElButton,
          { link: true, type: 'primary', size: 'small', onClick: () => handleViewActivationTransaction(row) },
          () => '交易详情'
        )
      }
    }
...
watch(
  ...
  (newId, oldId) => {
    if (newId !== oldId && newId) {
      console.log('ActivationDetails (operationView): orderId changed, fetching list for ID:', newId)
      if (activationCurrentPage.value !== 1) {
          activationCurrentPage.value = 1;
      }
      fetchActivationDetails()
