...
const batchOrderTableColumns = ref<TableColumn[]>([
  ...
  {
    prop: 'energy_price', field: 'energy_price', label: '能量单价', width: 100,
    formatter: (row) => row.energy_price ?? '-' // From sample data
  },
  {
    // !! 需要确认此字段 !! 使用 addr_energy_num 作为占位符 (from sample data)
    prop: 'addr_energy_num', field: 'addr_energy_num', label: '能量数', width: 100,
    formatter: (row) => row.addr_energy_num ?? '-'
  },
  {
    prop: 'active_price', field: 'active_price', label: '激活单价', width: 100,
    formatter: (row) => row.active_price ?? '-' // From sample data
  },
  {
    // !! 需要确认此字段 !! 使用 status 作为占位符 (from sample data)
    prop: 'status', field: 'status', label: '交易状态', width: 100, align: 'center',
    slots: { default: ({ row }) => getStatusTag('status', row.status) }
  },
  {
    prop: 'order_amount', field: 'order_amount', label: '扣款金额', width: 100,
...
.mt-20px { /* Ensure margin is defined if class is used */
    margin-top: 20px;
}
</style> 