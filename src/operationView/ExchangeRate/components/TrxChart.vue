      ...
      <div class="current-price-panel" v-if="currentPrice">
        <div class="price-display">
          ...
          <div class="price-compare">
            ...
            <div class="price-vs-yesterday" v-if="yesterdayClosePrice !== null">较昨日收盘: ${{ yesterdayClosePrice.toFixed(4) }}</div>
          </div>
        </div>
      </div>
...
import { ElCard, ElSkeleton, ElRadioGroup, ElRadioButton, ElDatePicker, ElButton } from 'element-plus'
...
const timeUtils = {
  ...
  createDateRange(startDate: Date, endDate: Date) {
    ...
    console.log(`创建日期范围: ${new Date(startTimestamp).toLocaleString()} 至 ${new Date(endTimestamp).toLocaleString()}`);
...
const filterDataByTimeRange = (startTimestamp: number, endTimestamp: number) => {
  console.log(`过滤数据，时间范围: ${new Date(startTimestamp).toLocaleString()} 至 ${new Date(endTimestamp).toLocaleString()}`);
