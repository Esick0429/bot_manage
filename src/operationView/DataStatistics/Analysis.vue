  ...
  try {
    ...
    if (responseData) {
      ...
      if (responseData.day_total) {
        const dailyData = responseData.day_total
        ...
        set(energyIncomeChartOptions, 'series', [{ name: '能量收入', data: incomeValues, type: 'bar' }]);
      ...
      if (responseData.last_week_user_num) {
        ...
        set(agentActivityChartOptions, 'series', [{
          name: '活跃数', type: 'line', smooth: true, data: numValues,
          label: { show: true, position: 'top' },
          itemStyle: { color: '#18BFFF' }, lineStyle: { width: 3 },
