...
import { getBotListApi, addBotApi, updateBotApi } from '@/api/credit/botlist/index'
    ...
    default: (data: any) => {
      ...
      return (
        <>
          ...
          <BaseButton type="warning" onClick={() => handlePriceConfig(row)}>
            价格配置
          </BaseButton>
        </>
      )
...
// 价格配置
const handlePriceConfig = (row) => {
  console.log('配置价格:', row)
  // 跳转到价格配置页面，携带 bot_id
  router.push({ name: 'BotPriceConfig', params: { botId: row.tg_bot_id } })
}

// 路由参数处理
const route = useRoute()
onMounted(() => {
  const botId = route.query.tg_bot_id
  if (botId && typeof botId === 'string') {
    searchTableRef.value?.setSearchParams({ tg_bot_id: botId })
    nextTick(() => {
